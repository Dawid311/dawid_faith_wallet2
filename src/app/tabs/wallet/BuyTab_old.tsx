...
export default function BuyTab() {
  ...
  // POL Onramp Funktion mit erweiterten Fallback-Strategien
  const handlePolOnramp = async () => {
    ...
    try {
      ...
      // Spezielle Behandlung für jeden Provider
      if (selectedProvider === "transak") {
        console.log("Transak: Verwende direkten Link (Bridge.Onramp Skip)");
        
        // Direkte Transak-URL erstellen
        const estimatedUSD = (parseFloat(polPurchaseAmount) * (polPriceEur ? polPriceEur * 1.1 : 0.65)).toFixed(2);
        const transakParams = new URLSearchParams({
          apiKey: 'YOUR_TRANSAK_API_KEY', // Placeholder - müsste durch echten API Key ersetzt werden
          hostURL: window.location.origin,
          walletAddress: account.address,
          cryptoCurrencyCode: 'MATIC',
          network: 'polygon',
          fiatCurrency: 'USD',
          fiatAmount: estimatedUSD,
        ...
        // Mock Session für Transak
        const mockSession = {
          id: `transak_${Date.now()}`,
          link: transakUrl,
          currency: "USD",
          currencyAmount: estimatedUSD,
          destinationAmount: polPurchaseAmount,
          provider: "transak"
        };
        
        setOnrampSession(mockSession);
        setPolOnrampStatus("ready");
        window.open(mockSession.link, '_blank');
        
      } else if (selectedProvider === "coinbase") {
        console.log("Coinbase: Versuche USDC als Alternative zu POL");
        
        try {
          // Erste Strategie: Versuche mit USDC Token
          const usdcAddress = "0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174"; // USDC on Polygon
          onrampPrepare = await Bridge.Onramp.prepare({
            client,
            onramp: selectedProvider,
            chainId: polygon.id,
            tokenAddress: usdcAddress, // USDC statt POL
            receiver: account.address,
            amount: toWei((parseFloat(polPurchaseAmount) * (polPriceEur ? polPriceEur * 1.1 : 0.65)).toString()), // USD Equivalent
            currency: "USD",
            onrampTokenAddress: usdcAddress,
            onrampChainId: polygon.id,
          });
          
          console.log("Coinbase USDC Onramp prepared:", onrampPrepare);
          setOnrampSession({...onrampPrepare, provider: "coinbase", note: "Kauft USDC - kann später zu POL getauscht werden"});
          
        } catch (usdcError) {
          console.log("Coinbase USDC Fehler, verwende direkten Coinbase Link:", usdcError);
          
          // Fallback: Direkter Coinbase Link
          const coinbaseUrl = `https://pay.coinbase.com/buy/select-asset?appId=YOUR_COINBASE_APP_ID&destinationWallets=[{"address":"${account.address}","assets":["MATIC"],"supportedNetworks":["polygon"]}]`;
          
          const mockSession = {
            id: `coinbase_${Date.now()}`,
            link: coinbaseUrl,
            currency: "USD",
            currencyAmount: (parseFloat(polPurchaseAmount) * (polPriceEur ? polPriceEur * 1.1 : 0.65)).toFixed(2),
            destinationAmount: polPurchaseAmount,
            provider: "coinbase",
            note: "Direkter Coinbase Link"
          };
        ...
        // Stripe: Normale Bridge.Onramp Verarbeitung
        console.log("Stripe: Verwende Standard Bridge.Onramp");
        ...
        try {
          onrampPrepare = await Bridge.Onramp.prepare({
            ...
            receiver: account.address,
            amount: toWei(polPurchaseAmount),
            currency: "USD",
            onrampTokenAddress: NATIVE_TOKEN_ADDRESS,
            onrampChainId: polygon.id,
          });
          
          console.log("Stripe Onramp prepared:", onrampPrepare);
          setOnrampSession({...onrampPrepare, provider: "stripe"});
          window.open(onrampPrepare.link, '_blank');
          
        } catch (stripeError) {
          console.log("Stripe Fehler, verwende direkten Link:", stripeError);
          
          // Stripe Fallback
          const stripeUrl = `https://buy.stripe.com/test_YOUR_STRIPE_LINK?prefilled_email=${encodeURIComponent('')}&client_reference_id=${account.address}`;
      ...
      // Erweiterte Fehlerbehandlung mit benutzerfreundlichen Alternativen
      let errorMessage = "Unbekannter Fehler";
      let fallbackAction = null;
      
      if (error.message?.includes("no route available")) {
        errorMessage = "Route nicht verfügbar";
        fallbackAction = () => {
          const fallbackUrls = {
          ...
          if (confirm(`${selectedProvider} unterstützt POL derzeit nicht direkt. Möchten Sie zur ${selectedProvider}-Website weitergeleitet werden?`)) {
            window.open(fallbackUrls[selectedProvider], '_blank');
          }
        };
      } else if (error.message?.includes("Currency EUR is not supported")) {
        errorMessage = "EUR nicht unterstützt";
      }
      
      setPolOnrampStatus("error");
      
      // Zeige Fallback-Option nach 2 Sekunden
      if (fallbackAction) {
        setTimeout(fallbackAction, 2000);
      }
      
      setTimeout(() => setPolOnrampStatus(null), 8000);
    } finally {
                ...
                >
                  ...
                  <div className="mb-4">
                    ...
                    <div className="mt-2 p-2 bg-zinc-800/50 rounded text-xs text-zinc-400">
                      {selectedProvider === "stripe" && "Kreditkarte, Debitkarte, Banking → Direkt POL (mit Fallback)"}
                      {selectedProvider === "coinbase" && "Coinbase Konto, Banking, Karte → USDC (tauschbar zu POL)"}
                      {selectedProvider === "transak" && "Global verfügbar, Banking, Karte → Direkter Link"}
                    </div>
                  </div>
                  ...
                  {selectedProvider === "coinbase" && (
                    <div className="mb-4 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded text-xs">
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          <div className="font-medium text-yellow-400 mb-1">Coinbase Alternative</div>
                          <div className="text-zinc-400">
                            Da POL nicht direkt unterstützt wird, kauft Coinbase USDC Token. Diese können Sie später in der App gegen POL tauschen. Als Fallback wird ein direkter Coinbase-Link verwendet.
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  {/* Transak Hinweis */}
                  {selectedProvider === "transak" && (
                    <div className="mb-4 p-3 bg-orange-500/10 border border-orange-500/30 rounded text-xs">
                      <div className="flex items-start gap-2">
                        <span className="text-orange-400 mt-0.5">⚠️</span>
                        <div>
                          <div className="font-medium text-orange-400 mb-1">Transak Direktlink</div>
                          <div className="text-zinc-400">
                            Aufgrund technischer Probleme mit der Transak-Integration wird ein direkter Link zur Transak-Website verwendet. Der Kaufprozess findet extern statt.
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  {polOnrampStatus && (
                    <div className={`mb-4 p-3 rounded text-center text-sm ${
                      ...
                      {polOnrampStatus === "ready" && <div><b>✅ Bereit!</b><div className="mt-1">{selectedProvider.charAt(0).toUpperCase() + selectedProvider.slice(1)}-Seite wurde geöffnet</div></div>}
                      ...
                      {polOnrampStatus === "error" && <div><b>❌ Fehler</b><div className="mt-1">Fallback wird verwendet</div></div>}
                    </div>
                  )}
                  ...
                  <div className="space-y-2">
                    {!onrampSession ? (
                      ...
                      >
                        ...
                        {polOnrampLoading ? "Vorbereitung..." : `${polPurchaseAmount || "0"} POL kaufen (${selectedProvider})`}
                      </Button>
                    ) : (
                      <div className="space-y-2">
                        <Button
